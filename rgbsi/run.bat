...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f rgbsi-share
	call %MVN_EXEC% clean package -pl rgbsi-share,rgbsi-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill rgbsi-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f rgbsi-acs
	call %MVN_EXEC% clean package -pl rgbsi-integration-tests,rgbsi-platform,rgbsi-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl rgbsi-platform,rgbsi-integration-tests,rgbsi-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl rgbsi-platform,rgbsi-integration-tests
EXIT /B 0
:purge
